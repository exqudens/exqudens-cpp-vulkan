        ...
        public:
          ...
          void reCreateSwapchain(uint32_t width, uint32_t height) {
            try {
              ...
              camera.colorImageView.value.reset();
              camera.colorImage.value.reset();

              camera.colorImage = Image::builder()
                  .setPhysicalDevice(root.physicalDevice.value)
                  .setDevice(root.device.value)
                  .setCreateInfo(
                      vk::ImageCreateInfo()
              ...
              camera.colorImageView = ImageView::builder()
                  .setDevice(root.device.value)
                  .setCreateInfo(
                      vk::ImageViewCreateInfo()
                          .setImage(*camera.colorImage.reference())
                          .setFormat(camera.colorImage.createInfo.format)
                          .setViewType(vk::ImageViewType::e2D)
                          .setFlags({})
                          .setComponents({})
                          .setSubresourceRange(
              ...
              camera.renderPass = RenderPass::builder()
                  ...
                  .addAttachment(
                      vk::AttachmentDescription()
                          .setFormat(camera.colorImage.createInfo.format)
                          .setSamples(camera.colorImage.createInfo.samples)
                      ...
                      GraphicsPipelineCreateInfo()
                          ...
                          .setMultisampleState(
                              vk::PipelineMultisampleStateCreateInfo()
                                  .setRasterizationSamples(camera.colorImage.createInfo.samples)
              ...
              for (size_t i = 0; i < camera.swapchainFramebuffers.size(); i++) {
                camera.swapchainFramebuffers.at(i) = Framebuffer::builder()
                    .setDevice(root.device.value)
                    .addAttachment(*camera.colorImageView.reference())
